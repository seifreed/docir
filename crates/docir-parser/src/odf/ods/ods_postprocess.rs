...
pub(super) fn emit_full_row_cells(
    ...
    for cell_data in &row_cells.cells {
        if cell_data.is_covered || !cell_data.should_emit() {
            col_idx = col_idx.checked_add(cell_data.col_repeat).ok_or_else(|| {
                ParseError::InvalidStructure("ODF cell column index overflow".to_string())
            })?;
            continue;
        }
