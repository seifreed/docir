    ...
    pub fn parse_reader<R: Read + Seek>(
        ...
        if zip.contains("content.xml") {
            ...
            if xml_bytes.is_empty() {
                ...
                let content_result = if use_parallel {
                    ...
                    parse_content_spreadsheet_parallel(
                        &xml_bytes,
                        &mut store,
                        &limits,
                        &self.config,
        ...
        if fast_mode && format == DocumentFormat::OdfSpreadsheet {
            if let Some(bytes) = content_bytes.as_deref() {
                let chunks = extract_spreadsheet_table_chunks(bytes);
                for (idx, chunk) in chunks.iter().enumerate() {
                    let sheet_name = table_name_from_chunk(&chunk.bytes, (idx + 1) as u32);
                    let path = format!(
                        "content.xml#sheet:{}@{}-{}",
...
fn parse_manifest(xml: &str) -> Result<Vec<OdfManifestEntry>, ParseError> {
        ...
        match reader.read_event_into(&mut buf) {
            Ok(Event::Start(e)) => match e.name().as_ref() {
                b"manifest:file-entry" => {
                    let path = attr_value(&e, b"manifest:full-path").unwrap_or_default();
                    let media_type = attr_value(&e, b"manifest:media-type");
                ...
                b"manifest:encryption-data" => {
                    if let Some(entry) = current_entry.as_mut() {
                        ...
                        enc.checksum_type = attr_value(&e, b"manifest:checksum-type");
                        enc.checksum = attr_value(&e, b"manifest:checksum")
                ...
                b"manifest:algorithm" => {
                    if let Some(entry) = current_entry.as_mut() {
                        ...
                        enc.algorithm_name = attr_value(&e, b"manifest:algorithm-name");
                        enc.init_vector = attr_value(&e, b"manifest:initialisation-vector")
                            .and_then(|v| decode_base64_bytes(&v));
                        enc.key_size = attr_value(&e, b"manifest:key-size")
                ...
                b"manifest:key-derivation" => {
                    if let Some(entry) = current_entry.as_mut() {
                        ...
                        enc.key_derivation_name = attr_value(&e, b"manifest:key-derivation-name");
                        enc.salt =
                            attr_value(&e, b"manifest:salt").and_then(|v| decode_base64_bytes(&v));
                        enc.iteration_count = attr_value(&e, b"manifest:iteration-count")
                            .and_then(|v| v.parse::<u32>().ok());
                        entry.encryption = Some(enc);
                    }
                }
...
fn parse_content(
    ...
    match format {
        DocumentFormat::OdfText => parse_content_text(xml, store, limits),
        DocumentFormat::OdfSpreadsheet => parse_content_spreadsheet(xml, store, limits),
        DocumentFormat::OdfPresentation => parse_content_presentation(xml, store, limits),
...
fn parse_content_spreadsheet_fast(
    xml: &[u8],
    store: &mut IrStore,
    limits: &dyn OdfLimitCounter,
) -> Result<OdfContentResult, ParseError> {
...
fn parse_content_spreadsheet_parallel(
    xml: &[u8],
    store: &mut IrStore,
...
fn extract_spreadsheet_table_chunks(xml: &[u8]) -> Vec<OdfTableChunk> {
...
fn table_name_from_chunk(chunk: &[u8], sheet_id: u32) -> String {
...
fn skip_element(reader: &mut OdfReader<'_>, end: &[u8]) -> Result<(), ParseError> {
    let mut buf = Vec::new();
...
fn is_manifest_entry_encrypted(entry: &OdfManifestEntry) -> bool {
...
fn manifest_has_encryption(entries: &[OdfManifestEntry]) -> bool {
    entries.iter().any(is_manifest_entry_encrypted)
}

fn encrypted_manifest_entries(entries: &[OdfManifestEntry]) -> Vec<String> {
    entries
        .iter()
...
fn format_odf_encryption_metadata(entry: &OdfManifestEntry) -> Option<String> {
    ...
    let checksum = enc
        ...
        .unwrap_or_else(|| "unknown".to_string());
    Some(format!(
        "Encrypted ODF part metadata: path={}, algorithm={}, kdf={}, key_bits={}, iterations={}, iv={}, salt={}, checksum={}",
        entry.path, algorithm, kdf, key_bits, iterations, iv, salt, checksum
    ))
}
...
fn parse_paragraph(
    ...
    Ok(build_paragraph(store, &text, numbering, outline_level))
}
        ...
        match reader.read_event_into(&mut buf) {
            ...
            Ok(Event::Empty(e)) => match e.name().as_ref() {
                ...
                b"text:p" | b"text:h" => {
                    ...
                    let paragraph_id = build_paragraph(store, "", numbering, outline_level);
                    content.extend(pending_inline_nodes.drain(..));
                    content.push(paragraph_id);
                }
                _ => {}
