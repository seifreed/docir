...
#[test]
fn test_parse_vml_picture_rejects_non_finite_style_lengths() {
    let xml = r#"
        <w:r xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"
             xmlns:v="urn:schemas-microsoft-com:vml"
             xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
          <w:pict>
            <v:shape id="_x0000_s1026" name="BadVML"
                     style="left:NaNpt;top:infmm;width:NaNcm;height:infinityin">
              <v:imagedata r:id="rIdImg"></v:imagedata>
            </v:shape>
          </w:pict>
        </w:r>
...
fn test_parse_vml_picture_reports_malformed_attributes() {
    let xml = r#"
        <w:r xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"
          ...
          <w:pict>
            <v:shape name="VMLPic">
              <v:imagedata r:id="rIdImg" r:id="rIdOther"></v:imagedata>
            </v:shape>
          </w:pict>
        </w:r>
        "#;
    ...
    let err = loop {
        match reader.read_event_into(&mut buf) {
            Ok(Event::Start(e)) if e.name().as_ref() == b"w:r" => {
                break parse_run(&mut parser, &mut reader, &rels).err();
            }
    ...
    match err.expect("malformed VML attributes must fail") {
...
fn parse_run_reports_unclosed_text_node() {
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            Ok(Event::Start(e)) if e.name().as_ref() == b"w:r" => {
                err = match parse_run(&mut parser, &mut reader, &rels) {
            ...
            Ok(Event::Eof) => break,
            Err(e) => panic!("xml error before run: {e}"),
            _ => {}
        }
        buf.clear();
    }

    match err.expect("run error") {
        ParseError::Xml { file, .. } => assert_eq!(file, "word/document.xml"),
        other => panic!("unexpected error: {other:?}"),
    }
}
