...
#[test]
fn test_docx_reports_malformed_comments_part() {
    let body = r#"
        <w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
          <w:body>
    ...
    let err = parser
        .parse_file(&path)
        .expect_err("malformed DOCX comments part must fail instead of being ignored");
    std::fs::remove_file(path).ok();

    match err {
        ParseError::Xml { file, .. } => {
            assert_eq!(file, "word/comments.xml");
        }
        other => panic!("expected XML error, got {other}"),
    }
}
...
#[test]
fn test_docx_reports_malformed_glossary_part() {
    let body = r#"
        <w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
          <w:body>
            <w:p><w:r><w:t>Glossary host</w:t></w:r></w:p>
          </w:body>
        </w:document>"#;
    ...
    let err = parser
        .parse_file(&path)
        .expect_err("malformed DOCX glossary part must fail instead of being ignored");
    std::fs::remove_file(path).ok();

    match err {
        ParseError::Xml { file, .. } => {
            assert_eq!(file, "word/glossary/document.xml");
        }
        other => panic!("expected XML error, got {other}"),
    }
}
