...
impl OoxmlParser {
    ...
    fn parse_shared_parts<R: Read + Seek>(
        &self,
        zip: &mut SecureZipReader<R>,
        content_types: &ContentTypes,
        store: &mut IrStore,
        ...
        for rel_path in rel_paths {
            ...
            let graph = shared_parts::build_relationship_graph(&source, &rel_path, &rels);
        ...
        for path in theme_paths {
            ...
            let mut theme = shared_parts::parse_theme(&theme_xml, &path)?;
        ...
        for path in media_paths {
            let data = zip.read_file(&path)?;
            let media_type = shared_parts::classify_media_type(&path);
        ...
        if doc_format == DocumentFormat::WordProcessing && zip.contains("word/people.xml") {
            ...
            let people = shared_parts::parse_people_part(&xml, "word/people.xml")?;
            let id = people.id;
        ...
        if doc_format == DocumentFormat::WordProcessing {
            ...
            for path in ext_paths {
                if path.ends_with("taskpanes.xml") {
                    let xml = zip.read_file_string(&path)?;
                    let panes = shared_parts::parse_web_extension_taskpanes(&xml, &path)?;
                    for pane in panes {
                        let id = pane.id;
                    ...
                    seen_parts.insert(path);
                } else if path.contains("webExtension") {
                    let xml = zip.read_file_string(&path)?;
                    let ext = shared_parts::parse_web_extension(&xml, &path)?;
                    let id = ext.id;
        ...
        // VML drawings (legacy)
        let vml_paths: Vec<String> = zip
            .file_names()
            .filter(|p| p.ends_with(".vml"))
            .map(|s| s.to_string())
            .collect();
        for path in vml_paths {
            ...
            let xml = zip.read_file_string(&path)?;
            let rels = self.read_part_relationships(zip, &path);
            let (mut drawing, shapes) = shared_parts::parse_vml_drawing(&xml, &path, &rels)?;
            drawing.span = Some(SourceSpan::new(&path));
            let mut shape_ids = Vec::new();
            for mut shape in shapes {
                shape.span = Some(SourceSpan::new(&path));
                let shape_id = shape.id;
                store.insert(IRNode::VmlShape(shape));
                shape_ids.push(shape_id);
            }
            drawing.shapes = shape_ids;
            let drawing_id = drawing.id;
            store.insert(IRNode::VmlDrawing(drawing));
            shared_ids.push(drawing_id);
            seen_parts.insert(path);
        }

        // DrawingML parts
        if doc_format == DocumentFormat::WordProcessing || doc_format == DocumentFormat::Spreadsheet
        {
            let prefix = if doc_format == DocumentFormat::WordProcessing {
                "word/drawings/"
            } else {
                "xl/drawings/"
            };
            let drawing_paths: Vec<String> = zip
                .file_names()
            ...
            for path in drawing_paths {
                ...
                let xml = zip.read_file_string(&path)?;
                let rels = self.read_part_relationships(zip, &path);
                let (mut drawing, shapes) = shared_parts::parse_drawingml_part(&xml, &path, &rels)?;
                drawing.span = Some(SourceSpan::new(&path));
                let mut shape_ids = Vec::new();
                for mut shape in shapes {
                    shape.span = Some(SourceSpan::new(&path));
                ...
                drawing.shapes = shape_ids;
                let drawing_id = drawing.id;
                store.insert(IRNode::DrawingPart(drawing));
                shared_ids.push(drawing_id);
                seen_parts.insert(path);
            }
        }
        ...
        for path in custom_xml_paths {
            ...
            let part = shared_parts::parse_custom_xml_part(&xml, &path, data.len() as u64)?;
            let part_id = part.id;
        ...
        for path in sig_paths {
            let xml = zip.read_file_string(&path)?;
            let sig = shared_parts::parse_signature(&xml, &path)?;
            let sig_id = sig.id;
            store.insert(IRNode::DigitalSignature(sig));
            shared_ids.push(sig_id);
            seen_parts.insert(path);
        }

        // Legacy parts (VML, ink, printer settings)
        let legacy_paths: Vec<String> = zip
            .file_names()
            .filter(|p| {
                p.ends_with(".vml")
                    || p.starts_with("word/ink/")
                    || p.starts_with("ppt/printerSettings/")
            })
            .map(|s| s.to_string())
            .collect();
        for path in legacy_paths {
            ...
            let data = zip.read_file(&path)?;
            let mut part = shared_parts::legacy_extension_part(&path, data.len() as u64);
            part.content_type = content_types.get_content_type(&path).map(|s| s.to_string());
            part.span = Some(SourceSpan::new(&path));
            let part_id = part.id;
            store.insert(IRNode::ExtensionPart(part));
...
fn rels_source_from_path(rels_path: &str) -> String {
    ...
    if let Some(pos) = rels_path.rfind("/_rels/") {
        ...
        let rest = &rels_path[pos + "/_rels/".len()..];
        if let Some(stripped) = rest.strip_suffix(".rels") {
            if prefix.is_empty() {
